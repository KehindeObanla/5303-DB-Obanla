...
<div >
  ...
  <form v-on:submit.prevent="doboth" class="Addadvsing" >
    ...
    <select name="Classification" id="Classification" required v-model="tosend.Classification">
...
methods:{
  doboth(){
   ...
   getData() {
        axios.post('http://143.244.153.25:8004/AddForm/',JSON.stringify(this.tosend),{headers: {'Content-Type': 'application/json'}}
        ) .then((res) => {
    ...
    reset(){
                ...
                for(let field in this.addstuff){
                    this.addstuff[field] = '';
                }
  ...
  getCrn(){
    ...
    axios.post('http://143.244.153.25:8004/maketable',JSON.stringify(array),{headers: {'Content-Type': 'application/json'}}
        ) .then((res) => {
                     //Perform Success Action
                     // JSON responses are automatically parsed.
